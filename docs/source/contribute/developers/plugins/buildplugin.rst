...
will appear below the plugin name on the settings page. This is normally 1 scentence describing what the plugin
...
With the ability for the user to enter the details you require to retrive the OAuth tokens you then need to write the
code to retrive them, this is normally done in a function called setUpPluginNameInteractions() in the
PluginConfigurationController class.

The first thing you need to do in this function is to retrive the values of any form data the user has entered
...
link the user needs to go to in order to retrive the OAuth tokens.
...
to verify that the OAuth tokens you have are still valid and if not refresh them and retrive more detailed information
about the instance user in the process.

Then it is time to write the main method for your plugin, the one which actually goes out to the service and retrives
...
In this method you will need to get the plugin options so you have access to the OAuth tokens  and then retrive the
